...
- **Default:** implement features/fixes on a **separate** git branch in a **separate** git worktree — never on `main`/`master` directly. Exception: Personal repos with convention of commiting directly to main. 
...
- **Worktree location:** place every new worktree of `<project_path>` at `<project_path>-worktrees/<worktree_name>` - **never** nested inside the repo. E.g. `~/dev/xyz` -> `~/dev/xyz-worktrees/my-fix`.
- **PRs:** draft PRs are fine to open when there's a fix or feature worth reviewing; never open a non-draft PR or mark one ready-for-review yourself.
- **Commits during iteration:** push plain incremental commits (`git commit` + `git push`); **never** force-push or amend. 
...
- **Run it E2E** through the real entry point (within reason).
- **Prove each discovery** with **hard**, beyond-doubt reproductions. The exact same repro must be re-run after fixes.
- **Run CI locally** — format, lint, typecheck, test, build.
- Never assume your fix works without thorough verification.
...
- **Prose assymetry**: Logic lacking or contradicting the docs - cross-verify.
- **Symmetry** - spawn/reap, open/close, write/delete, set/reset, and similar.
- **Mutations** - break the constant, delete the branch, invert the condition, swap two args, run the suite; green means nothing pins it. Highest-yield single act, and it applies to every change, not just fixes.
...
Dispatch independent work to sub-agents aggressively, including swarms of them. Any task that doesn't require massive shared context or exclusive access to a race-prone resource (a single Android AVD, a single dev port, an in-progress DB migration, an interactive shell session) should be delegated. File searches across the repo, isolated edits to unrelated files, build verifications, independent test suites, multi-file refactors with non-overlapping scope, research and exploration: all of these run faster as parallel sub-agents than serially. Default to delegating; reserve the main-thread context for synthesis, decisions, and work that must stay coherent. The cost of an unnecessary agent is small; the cost of unnecessarily serializing parallelizable work is paid against the user's wall-clock time. **Swarm sizing counts agents running at the same time, never agents in total.** 2-8 concurrent is the operating range - size within it to match the task - and 8 is a hard ceiling nothing exceeds, workflows included. Total headcount is not capped: a workflow may spawn any number of agents over its lifetime, and a `pipeline()` over 200 items is fine precisely because at most 8 of them run at once. What is forbidden is 9+ agents executing simultaneously - out of one `parallel()`, out of several workflows running side by side, or out of ad-hoc Agent calls. The ceiling is per-session, not per-workflow: concurrent workflows share the same 8 between them. Under-provisioning a parallelizable task wastes wall-clock time as surely as serializing it.

**Workflows over raw swarms:** any run spawning more than 5 sub-agents *in total* - the one count in this section that is headcount rather than concurrency - must be orchestrated via the Workflow tool, not ad-hoc parallel Agent calls.
...
- Bring real analytical depth to non-trivial decisions: failure modes, second-order effects, fit with existing patterns. 
- `AskUserQuestion` is not an escape valve. Within a task the user delegated, every strategic choice is yours — they have less context on your immediate problem than you do at that moment, and asking is itself the failure.
...
Severity is **H/M/L**, below which sits **nitpick** — style preference, phrasing, or a defect with no reachable consequence.
Beyond severity, non-bug findings may use category tags: **S** scope/simplification, **T** tests, **D** docs.
